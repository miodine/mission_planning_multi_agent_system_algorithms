...
def estimate_desired_heading(x_r, y_r, x_g, y_g):
    '''
    Function for estimating desired orientation wrt to the current goal.
    '''
...
def estimate_global_goal_coords(robot):
    '''
    Function for estimating global coordinates of the flag.
    '''
...
def _cdt_obs(robot,obstacle):
    '''
    A sort of heuristic garbage collector - delete obstacle from memory if robot is 
    far from the obstacle. 
    '''

    # If the variable is of None type - return it.  
    if obstacle is None:
        return None

    # Else:
    # helper functions to compute vector pointing to./ and distance to the obstacle 
    vec_to_obs = lambda r_pos,o_pos: (o_pos[0]-r_pos[0], o_pos[1]-r_pos[1])
    vec_len = lambda vec: math.sqrt(vec[0]**2 + vec[1]**2)
    v = vec_to_obs(robot, obstacle)
    d = vec_len(v)

    # if robot is far from the obstacle -> clear the variable from workspace
    if d>5.0:
        return None
    # otherwise keep the obstacle in memory
    else:
        return obstacle


def force_field_adjust(a_des, robot_pos, obstacle):
    '''
    Adjust heading angle in such a way that the robot is 
    'repelled' from the obstacle. It is a sort of quasi APF method.
    '''

    # if there is no obstacle in the robot vicinity - return heading 
    # obtained on the basis of global goal.
    if obstacle is None:
        return a_des
    
    # helper functions
    vec_to_obs = lambda r_pos,o_pos: (o_pos[0]-r_pos[0], o_pos[1]-r_pos[1]) # robot to obstacle vector
    vec_len = lambda vec: math.sqrt(vec[0]**2 + vec[1]**2)                  # distance to the obstacle (euc. norm of 2D vector)
    obs_cstrt = lambda val,sym_cstrt: max(min(val,sym_cstrt), -sym_cstrt)   # saturation constraint

    # compute vector to pointing to the obstacle and distance to it 
    v = vec_to_obs(robot_pos, obstacle)
    d = vec_len(v)

    # compute angle to deflect from the robot's course based on the vector to the obstacle 
    a_rep = math.atan2(-v[1], -v[0])        # get the angle of vector determining the direction from the obstacle to the robot 
    a_rep = (d/5.0)*obs_cstrt(a_rep,1.57/2) # scale this angle based on how far it is from the origin of the 'repulsive field'
                                            # (which is the obstacle) - and constrain it so that it wouldn't be greater than 90 deg.

    # alter the trajectory based on current heading of the robot
    # if robot is approaching goal at a positive angle 
    if a_rep >= 0:
        a_des -= a_rep # subtract the computed deflection angle
    else:
        a_des += a_rep # otherwise - add it
...
def run_strategy_2():
    ...
    while not rospy.is_shutdown():
        ...
        # sometimes the simulation fails to obtain distance-to-flag
        try:
            dtf = float(robot.getDistanceToFlag())
        except:
            pass 
        ...
        # -- run strategy --
        if snr_obs < obstacle_detection_threshold: # get coordinates of the obstacle
            x_obs = x + snr_obs*math.cos(theta)
            y_obs = y + snr_obs*math.sin(theta)
            obstacle = (x_obs,y_obs)

        obstacle = _cdt_obs(robot_pos, obstacle)
        angle_desired = force_field_adjust(angle_desired, robot_pos, obstacle)

        # -- assign angle conditioning -- 
