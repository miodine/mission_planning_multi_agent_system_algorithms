...
class robotPID:
    def __init__(self, dt, tunings: list):
        self.dt = dt
        self.kp = tunings[0]
        self.ki = tunings[1]
        self.kd = tunings[2]
    ...
    def pid_dist_to_velo(self, distance):
        ...
        proportional_term = e*self.kp
        integral_term = self.ki*(e+self.e_z_1)
        derivative_term = self.kd*(e-self.e_z_1)/self.dt
        self.e_z_1 = e
...
def run_demo():
    ...
    # controller setup
    tunings = [10, 5, 1]
    robot_controller = robotPID(dt, tunings)

    # Timing
    delay_base = 2
    rospy.sleep(delay_base*int(robot_name[-1]))
    ...
    while not rospy.is_shutdown():
        ...
        angle = 0
        velocity = robot_controller.pid_dist_to_velo(distance)
